...
## 更新日志
- **2025.11.30** (hotfix)增加raycast demo的config配置相关说明, add back区域赛场地mesh文件
...
pip install open3d
```
1. 🔍 找到 `config/params.yaml` ，根据其中注释指示将实际上场相机参数与场地相关配置 (`transform.K, transform.dist_coffs, mesh_path, keypoints`) 替换为 demo 用参数
